...
ParticleGenerator::populateTypeInfo(vector<string>& tokens){
  ...
  if(typeName == "Point"){
    ...
    type = StarType;
    rectangle.lowerLeftPoint = glm::vec3(atof(tokens[3].c_str()), 
      atof(tokens[4].c_str()), atof(tokens[5].c_str()));
    rectangle.width = atof(tokens[7].c_str());
    rectangle.height = atof(tokens[9].c_str());
    rectangle.normal = glm::vec3(atof(tokens[11].c_str()), 
      atof(tokens[12].c_str()), atof(tokens[13].c_str()));
  } 
}
...
//////////////////////////////////////////////////////////////////////////////
// @brief: Generate particles based on a uniform distribution, from a rectangular surface 
void ParticleGenerator::starGenerate(){
  glm::vec3 position;
  glm::vec3 velocity;
  unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::default_random_engine generator (seed);
  ...
  std::uniform_int_distribution<int> distributionLongevity(minimumLife, 
    maximumLife);
  std::uniform_real_distribution<double> distributionY(0, rectangle.height);
  std::uniform_real_distribution<double> distributionZ(0, rectangle.width);

  for(int i = 0; i < numGenerated; i++){
    float randY = rectangle.lowerLeftPoint[1] + distributionY(generator);
    float randZ = rectangle.lowerLeftPoint[2] + distributionZ(generator);
    position = glm::vec3(rectangle.lowerLeftPoint[0], randY, randZ);
